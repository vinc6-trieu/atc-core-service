...
var fdSlider = (function () {
      ...
      if (typeof JSON === 'object' && JSON.parse) {
        ...
        var f = Function(
          [
            ...
            'return (',
            str.replace(/<\!--.+-->/gim, '').replace(/\bfunction\b/g, 'function-'),
            ');',
          ].join(''),
        )
        return f()
  ...
  var createSlider = function (options) {
    ...
    if (options.inp.tagName.toLowerCase() == 'select') {
      ...
      options.step =
        options.step && String(options.step).search(stepRegExp) != -1 ? options.step : 1
      options.precision =
  ...
  function fdRange(options) {
    ...
    function redraw() {
      ...
      try {
        ...
        stepPx = mPx / steps
        rMinPx = Math.max(
          scale ? percentToPixels(valueToPercent(rMin)) : Math.abs((rMin - min) / step) * stepPx,
          0,
        )
        rMaxPx = Math.min(
          scale ? percentToPixels(valueToPercent(rMax)) : Math.abs((rMax - min) / step) * stepPx,
    ...
    function trackMouse(e) {
      ...
      pixelsToValue(
        snapToPxValue(handlePos + (vertical ? e.clientY - mousePos : e.clientX - mousePos)),
      )
