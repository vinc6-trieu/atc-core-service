...
describe('Time scale tests', function () {
  ...
  describe('when rendering several days', function () {
    ...
    it('should convert between screen coordinates and times', function () {
      ...
      var lastPointOffsetMs =
        moment(chart.config.data.labels[chart.config.data.labels.length - 1]).valueOf() - scale.min
  ...
  describe('when rendering several years', function () {
    ...
    it('should build the correct ticks', function () {
      ...
      expect(getTicksLabels(this.scale)).toEqual([
        '2005',
        '2007',
        '2009',
        '2011',
        ...
        datasets: [
          {
            xAxisID: 'xScale0',
            data: [
              { t: '2015-01-01T20:00:00', y: 10 },
              { t: '2015-01-02T21:00:00', y: 3 },
            ],
          },
        ],
      },
  ...
  describe('when ticks.source', function () {
    describe('is "labels"', function () {
      ...
      it('should generate ticks from "data.labels"', function () {
        ...
        expect(getTicksLabels(scale)).toEqual(['2017', '2019', '2020', '2025', '2042'])
      })
    ...
    describe('is "data"', function () {
      ...
      it('should correctly handle empty `data.labels` using "day" if `time.unit` is undefined`', function () {
        ...
        expect(scale.max).toEqual(+moment('2043', 'YYYY'))
        expect(getTicksLabels(scale)).toEqual(['2018', '2020', '2043'])
      })
  ...
  describe('when bounds', function () {
    describe('is "data"', function () {
      it('should preserve the data range', function () {
        ...
        expect(getTicksLabels(scale)).toEqual(['Feb 21', 'Feb 22', 'Feb 23'])
      })
    })
    ...
    describe('is "labels"', function () {
      it('should preserve the label range', function () {
        ...
        expect(getTicksLabels(scale)).toEqual(['Feb 20', 'Feb 21', 'Feb 22', 'Feb 23', 'Feb 24'])
      })
    })
  })
  ...
  ;['auto', 'data', 'labels'].forEach(function (source) {
    ;['series', 'linear'].forEach(function (distribution) {
      describe(
        'when ticks.source is "' + source + '" and distribution is "' + distribution + '"',
        function () {
          beforeEach(function () {
            this.chart = window.acquireChart({
              type: 'line',
          ...
          it('should add offset from the edges if offset is true', function () {
            ...
            var lastTickInterval =
              scale.getPixelForTick(numTicks - 1) - scale.getPixelForTick(numTicks - 2)

            expect(scale.getPixelForValue('2017')).toBeCloseToPixel(
              scale.left + firstTickInterval / 2,
            )
            expect(scale.getPixelForValue('2042')).toBeCloseToPixel(
              scale.left + scale.width - lastTickInterval / 2,
            )
          })
